...
for n,i in enumerate(['SVM', 'RF', 'KNN', 'XGBoost', 'DNN', 'CNN', 'LSTM']):
    ...
    acc, auc, sn, sp, mcc = evaluate_metrics(train_label, pred_train)
    results.append([f'{i}_train', acc, auc, sn, sp, mcc])
    acc, auc, sn, sp, mcc = evaluate_metrics(test_label, pred_test)
    fpr, tpr, _ = roc_curve(test_label, pred_test)
    tprs.append(tpr)
    fprs.append(fpr)
    final_auc.append([fpr, tpr, auc])
    results.append([f'{i}_test', acc, auc, sn, sp, mcc])
