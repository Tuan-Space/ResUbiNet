...
def bootStrapping_allneg_continue_keras2(trainfile,valfile=None,srate=0.8,
  ...
  print('training_data:', train_pos.shape, train_neg.shape)
  ...
  if(valfile is not None): # use all data in valfile as val
     ...
     print('val_data:', val_pos.shape, val_neg.shape)
  ...
  if(maxneg is not None):
       nclass=max(min(maxneg, nclass), 1); #cannot do more than maxneg times ; make sure at least il
  #modelweights=None;
